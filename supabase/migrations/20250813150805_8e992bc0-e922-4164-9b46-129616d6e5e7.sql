...
INSERT INTO public.awards (school_id, title, description, date)
SELECT s.id, v.title, v.description, v.date FROM public.schools s
CROSS JOIN (VALUES 
...
SELECT s.id, v.semester, v.year, v.display_order FROM public.schools s
CROSS JOIN (VALUES 
  ('Fall', 2023, 0),
  ('Spring', 2024, 1),
  ('Summer', 2024, 2),
...
SELECT s.id, v.code, v.name, v.credits, v.description, v.tools FROM public.semesters s
